...
export default async function exportPageData(records, req:HttpRequest, res:HttpResponse) {
	try{
		...
		let columns =  [
				...
				{ header: "Certificate Name", key: "certificate_name" },
				{ header: "Student Id", key: "student_id" },
				{ header: "Award Date", key: "award_date" },
				{ header: "Blocknum", key: "blocknum" },
				{ header: "Tx Hash", key: "tx_hash" },
				{ header: "Certificate Photo", key: "certificate_photo" },
				{ header: "Users Username", key: "users_username" },
				{ header: "Users Usertele", key: "users_usertele" }
		]
		let filename = "certificateslist-report";
		if(format == "excel"){
			let workbook = new excel.Workbook();
			let worksheet = workbook.addWorksheet("Certificates");
