...
export default async function exportPageData(records, req:HttpRequest, res:HttpResponse) {
	try{
		...
		let columns =  [
				...
				{ header: "Study Date", key: "study_date" },
				{ header: "Study Duration", key: "study_duration" }
		]
		let filename = "studyrecordslist-report";
		if(format == "excel"){
			let workbook = new excel.Workbook();
			let worksheet = workbook.addWorksheet("Study Records");
