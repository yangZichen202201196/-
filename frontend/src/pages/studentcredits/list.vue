            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('studentcredits/add')">
                                <router-link :to="`/studentcredits/add`">
                                    <Button :label="$t('addNewStudentCredit')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/studentcredits'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="student_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="student_id" :header="$t('studentId')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('users')" v-if="data.student_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.student_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        <Column  field="total_credits" :header="$t('totalCredits')" >
                                            <template #body="{ data, index }">
                                                {{ data.total_credits }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="grid justify-content-between align-items-center">
                                        <div class="flex gap-2 flex-grow-0">
                                            <template v-if="auth.canView('studentcredits')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'studentcreditslist',
		},
		apiPath: {
			type: String,
			default: 'studentcredits/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/studentcredits/view/${data.student_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('studentcredits/view')
		},
		{
			label: () => $t('edit'),
			to: `/studentcredits/edit/${data.student_id}`,
			icon: "pi pi-pencil",
	...
	onMounted(()=>{ 
		const pageTitle = $t('studentCredits');
