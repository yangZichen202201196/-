            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('collents/add')">
                                <router-link :to="`/collents/add`">
                                    <Button :label="$t('addNewCollent')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/collents'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="collent_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="collent_id" :header="$t('collentId')" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/collents/view/${data.collent_id}`">
                                                    {{ data.collent_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="id" :header="$t('id')" >
                                            <template #body="{ data, index }">
                                                <Button class="p-button-text" icon="pi pi-eye" :label="$t('courses')" v-if="data.id" @click="app.openPageDialog({ page: 'courses/view', url: `/courses/view/${data.id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'collentslist',
		},
		apiPath: {
			type: String,
			default: 'collents/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/collents/view/${data.collent_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('collents/view')
		},
		{
			label: () => $t('edit'),
			to: `/collents/edit/${data.collent_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('collents/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.collent_id) },
			icon: "pi pi-trash",
			visible: auth.canView('collents/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('collents');
