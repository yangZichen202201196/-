            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <template v-if="auth.canView('reply/add')">
                                <router-link :to="`/reply/add`">
                                    <Button :label="$t('addNewReply')" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                                </router-link>
                            </template>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/reply'}" :model="pageBreadCrumb" />
                            </template>
...
<script setup>
	...
	function getActionMenuModel(data){
		...
		{
			label: () => $t('view'),
			to: `/reply/view/${data.reply_id}`,
			icon: "pi pi-eye",
			visible: auth.canView('reply/view')
		},
		{
			label: () => $t('edit'),
			to: `/reply/edit/${data.reply_id}`,
			icon: "pi pi-pencil",
			visible: auth.canView('reply/edit')
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.reply_id) },
			icon: "pi pi-trash",
			visible: auth.canView('reply/delete')
		}
	]
	}
	
	onMounted(()=>{ 
		const pageTitle = $t('reply');
